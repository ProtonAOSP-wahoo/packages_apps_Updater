...
updates_dir=/data/protonaosp_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have protonaosp-VERSION-DATE-TYPE-* as format"
...
# Assume protonaosp-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.protonaosp.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.protonaosp.updater/databases/updates.db" \
